...
from modules.connection_tracker import ConnectionTracker
from modules.smart_filter import SmartFilter
from modules.analytics import Analytics
from modules.human_behavior import simulate_reading, human_like_scroll, typing_with_delays, random_pause
...
# Initialize External Application Managers
external_links_manager = ExternalLinksManager(external_links_file)
field_knowledge_base = FieldKnowledgeBase(knowledge_base_file)

# Initialize Enhancement Modules
if enable_hr_connections:
    connection_tracker = ConnectionTracker(daily_limit=daily_connection_limit)
else:
...
if enable_analytics:
    analytics = Analytics(analytics_file)
else:
    analytics = None
                        ...
                        if use_AI and aiClient:
                            try:
                                ...
                                if ai_provider.lower() == "openai":
                                    ai_answer = ai_answer_question(aiClient, label_org, options=optionsText, question_type="single_select", job_description=job_description, about_company=about_company_for_ai, user_information_all=user_information_all)
                                elif ai_provider.lower() == "deepseek":
                                    ai_answer = deepseek_answer_question(aiClient, label_org, options=optionsText, question_type="single_select", job_description=job_description, about_company=about_company_for_ai, user_information_all=user_information_all)
                                elif ai_provider.lower() == "gemini":
                                    ai_answer = gemini_answer_question(aiClient, label_org, options=optionsText, question_type="single_select", job_description=job_description, about_company=about_company_for_ai, user_information_all=user_information_all)
                                ...
                                if ai_answer:
                                    try:
                                        ...
                                        answer = ai_answer
                                        foundOption = True
                                        print_lg(f'AI selected option "{answer}" for question "{label_org}"')
                                    except:
                                        print_lg(f'AI suggested "{ai_answer}" but it was not found in options!')
                            except Exception as e:
            ...
            if not prev_answer or overwrite_previous_answers:
                ...
                if answer == "":
                    if use_AI and aiClient:
                        try:
                            if ai_provider.lower() == "openai":
                                answer = ai_answer_question(aiClient, label_org, question_type="text", job_description=job_description, about_company=about_company_for_ai, user_information_all=user_information_all)
                            elif ai_provider.lower() == "deepseek":
                                answer = deepseek_answer_question(aiClient, label_org, options=None, question_type="text", job_description=job_description, about_company=about_company_for_ai, user_information_all=user_information_all)
                            elif ai_provider.lower() == "gemini":
                                answer = gemini_answer_question(aiClient, label_org, options=None, question_type="text", job_description=job_description, about_company=about_company_for_ai, user_information_all=user_information_all)
                            else:
            ...
            if not prev_answer or overwrite_previous_answers:
                ...
                ##> ------ Yang Li : MARKYangL - Feature ------
                    if use_AI and aiClient:
                        try:
                            if ai_provider.lower() == "openai":
                                answer = ai_answer_question(aiClient, label_org, question_type="textarea", job_description=job_description, about_company=about_company_for_ai, user_information_all=user_information_all)
                            elif ai_provider.lower() == "deepseek":
                                answer = deepseek_answer_question(aiClient, label_org, options=None, question_type="textarea", job_description=job_description, about_company=about_company_for_ai, user_information_all=user_information_all)
                            elif ai_provider.lower() == "gemini":
                                answer = gemini_answer_question(aiClient, label_org, options=None, question_type="textarea", job_description=job_description, about_company=about_company_for_ai, user_information_all=user_information_all)
                            else:
...
def external_apply(pagination_element: WebElement, job_id: str, job_link: str, resume: str, date_listed, application_link: str, screenshot_name: str) -> tuple[bool, str, int]:
    '''
    Function to handle external job applications
    '''
    global tabs_count, dailyEasyApplyLimitReached
    
    # If Easy Apply only is strictly enforced and we failed to find it
    if easy_apply_only and not enable_external_applications:
        try:
        ...
        print_lg("Easy apply failed and external applications disabled.")
        if pagination_element != None: return True, application_link, tabs_count

    try:
        # Try to find the Apply button with multiple selectors
        apply_button = False
        if not apply_button:
            try:
                wait.until(EC.element_to_be_clickable((By.XPATH, ".//button[contains(@class,'jobs-apply-button') and .//span[normalize-space()='Apply']]"))).click()
                apply_button = True
            except: pass
        if not apply_button:
            try:
        ...
        if not apply_button: raise Exception("Could not find Apply button")
        
        # Handle "Continue" modal if it appears
        ...
        if enable_external_applications:
            try:
                print_lg("Attempting external application...")
                
                # Apply stealth
                apply_stealth(driver)
                
                # Check for CAPTCHA
                if should_skip_captcha_site(driver, skip_captcha_sites):
                    external_links_manager.save_external_link(job_id, "Unknown Title", "Unknown Company", application_link, "unknown", "CAPTCHA detected")
                    print_lg("Skipped due to CAPTCHA")
                else:
                    # Detect ATS
                    ats_platform = detect_ats_platform(driver)
                    print_lg(f"Detected ATS: {ats_platform}")
                    ...
                    if ats_platform == 'greenhouse':
                        ...
                        user_data = {
                            ...
                            'linkedin': linkedIn,
                            'website': website,
                            'github': website if 'github.com' in website.lower() else ''
                        }
                        
                        # Fill application
                        success, message = fill_greenhouse_application(
                            driver, 
                            user_data, 
                            default_resume_path, 
                        ...
                        if success:
                            print_lg(f"Greenhouse application result: {message}")
                        else:
                            external_links_manager.save_external_link(job_id, "Unknown Title", "Unknown Company", application_link, ats_platform, f"Failed: {message}")
                    else:
                        print_lg(f"ATS {ats_platform} not supported yet. Saving link.")
                        external_links_manager.save_external_link(job_id, "Unknown Title", "Unknown Company", application_link, ats_platform, "Unsupported ATS")
            
            except Exception as ex:
                print_lg(f"External application error: {ex}")
                external_links_manager.save_external_link(job_id, "Unknown Title", "Unknown Company", application_link, "error", str(ex))
...
def apply_to_jobs(search_terms: list[str]) -> None:
    ...
    global current_city, failed_count, skip_count, easy_applied_count, external_jobs_count, tabs_count, pause_before_submit, pause_at_failed_question, useNewResume, about_company_for_ai
    
    # Calculate session duration
    session_duration = randint(min_runtime_minutes, max_runtime_minutes) * 60
    ...
    for searchTerm in search_terms:
        ...
        current_time_filter = initial_time_filter if enable_dynamic_time_filter else "r600"
        filter_index = 0
        jobs_found = 0
        
        while True:
            # Build search URL with current time filter
            search_url = f"https://www.linkedin.com/jobs/search/?keywords={searchTerm}&f_TPR={current_time_filter}"
            driver.get(search_url)
            print_lg(f"\nSearching with time filter: {current_time_filter}")
            print_lg("\n________________________________________________________________________________________________________________________\n")
            print_lg(f'\n>>>> Now searching for "{searchTerm}" <<<<\n\n')

            apply_filters()
            
            # Ensure r600 is in the URL after filters are applied
            current_url = driver.current_url
            if "f_TPR" not in current_url:
                # Add r600 parameter if missing
                separator = "&" if "?" in current_url else "?"
                new_url = f"{current_url}{separator}f_TPR={current_time_filter}"
                driver.get(new_url)
                print_lg(f"Updated URL with time filter: {current_time_filter}")
                buffer(2)
            elif f"f_TPR={current_time_filter}" not in current_url:
                # Replace existing f_TPR parameter
                import re
                new_url = re.sub(r'f_TPR=[^&]*', f'f_TPR={current_time_filter}', current_url)
                driver.get(new_url)
                print_lg(f"Updated URL with time filter: {current_time_filter}")
                buffer(2)
            ...
            try:
                ...
                print_lg(f"Found {jobs_found} jobs with filter {current_time_filter}")
                
                # If we have enough jobs or no more fallback filters, proceed
                if jobs_found >= min_jobs_threshold or not enable_dynamic_time_filter:
                    break
                
                # Try next fallback filter
                if filter_index < len(fallback_filters):
                    current_time_filter = fallback_filters[filter_index]
                    filter_index += 1
                    print_lg(f"Too few jobs ({jobs_found}), expanding to {current_time_filter}...")
                else:
                    print_lg(f"No more fallback filters. Proceeding with {jobs_found} jobs.")
                    break
            except:
                break
            ...
            while current_count < switch_number:
                ...
                for job in job_listings:
                    ...
                    try:
                        ...
                        # Handle stale element or other errors during job detail extraction
                        print_lg(f"Failed to get job details (Stale Element?): {str(e)[:100]}")
                        continue
                    
                    if skip: continue
                    
                    # Smart filtering checks
                    if enable_smart_filtering and smart_filter:
                        # Skip if already viewed but not applied
                        if skip_viewed_jobs and smart_filter.is_viewed(job_id):
                            print_lg(f'Already viewed "{title} | {company}" (Job ID: {job_id}). Skipping.')
                            continue
                        
                        # Skip if company has rejected applications before
                        if skip_rejected_companies and smart_filter.should_skip_company(company, company_rejection_threshold):
                            print_lg(f'Company "{company}" has rejected applications before. Skipping.')
                            continue
                        
                        # Mark as viewed
                        smart_filter.mark_as_viewed(job_id)
                    ...
                    try:
                        rejected_jobs, blacklisted_companies, jobs_top_card, about_company_for_ai = check_blacklist(rejected_jobs,job_id,company,blacklisted_companies)
                    ...
                    try:
                        ...
                        # Log recruiter details
                        try:
                            import csv
                            log_dir = os.path.join(os.path.dirname(__file__), "logs")
                            os.makedirs(log_dir, exist_ok=True)
                            recruiter_log_file = os.path.join(log_dir, "recruiter_data.csv")
                            
                            # Check if file exists to write header
                            file_exists = os.path.isfile(recruiter_log_file)
                            
                            with open(recruiter_log_file, "a", newline="", encoding="utf-8") as f:
                                writer = csv.writer(f)
                                if not file_exists:
                                    writer.writerow(["Date", "Recruiter Name", "LinkedIn Profile", "Job Title", "Company", "Email"])
                                
                                # Try to find email in the text (simple regex)
                                import re
                                email_match = re.search(r'[\w\.-]+@[\w\.-]+', hr_info_card.text)
                                email = email_match.group(0) if email_match else "Not Found"
                                
                                writer.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), hr_name, hr_link, title, company, email])
                                print_lg(f"Saved recruiter info: {hr_name}")
                        except Exception as log_err:
                            print_lg(f"Failed to save recruiter info: {log_err}")

                        if connect_hr and connection_tracker:
                            try:
                                # Check if we can send a connection request
                                can_send, reason = connection_tracker.can_send_request()
                                if not can_send:
                                    print_lg(f"Skipping connection request: {reason}")
                                elif connection_tracker.is_already_connected(hr_link):
                                    print_lg(f"Already connected to {hr_name}")
                                else:
                                    # Wait if needed for rate limiting
                                    connection_tracker.wait_if_needed()
                                    
                                    driver.switch_to.new_window('tab')
                                    driver.get(hr_link)
                                    buffer(2)
                                    
                                    # Create optimized message (under 200 chars)
                                    # Format: "Hi, I'm Divyansh Dewangan. Interested in {title} at {company}. Would appreciate your referral. Thanks!"
                                    base_msg = "Hi, I'm Divyansh Dewangan. Interested in "
                                    suffix = ". Would appreciate your referral. Thanks!"
                                    max_job_company_length = 200 - len(base_msg) - len(suffix)
                                    
                                    # Truncate job title and company if needed
                                    job_company = f"{title} at {company}"
                                    if len(job_company) > max_job_company_length:
                                        # Try shortening job title
                                        max_title_length = max_job_company_length - len(company) - 4  # " at "
                                        truncated_title = title[:max_title_length-3] + "..." if len(title) > max_title_length else title
                                        job_company = f"{truncated_title} at {company}"
                                    
                                    personalized_message = base_msg + job_company + suffix
                                    
                                    # Ensure message is under 200 chars
                                    if len(personalized_message) > 200:
                                        personalized_message = personalized_message[:197] + "..."
                                    
                                    print_lg(f"Message length: {len(personalized_message)} chars")
                                    ...
                                    try:
                                        ...
                                        try:
                                            connected_indicator = driver.find_element(By.XPATH, "//span[contains(text(), 'Message') or contains(text(), 'Connected')]")
                                            print_lg(f"Already connected to {hr_name}")
                                            connection_tracker.add_connected_profile(hr_link)
                                            connection_success = True
                                        except:
                                            # Not connected, try to send request
                                            try:
                                                # Try direct Connect button
                                                wait_span_click(driver, "Connect", 2)
                                            except:
                                                # Try More -> Connect
                                                try:
                                                    wait_span_click(driver, "More", 2)
                                                    wait_span_click(driver, "Connect", 2)
                                                except:
                                                    # Try Follow as fallback
                                                    try:
                                                        wait_span_click(driver, "Follow", 2)
                                                        print_lg(f"Followed {hr_name} (Connect not available)")
                                                        connection_tracker.record_request(hr_name, hr_link, title, company, True, "Followed instead")
                                                        connection_success = True
                                                    except:
                                                        raise Exception("No Connect or Follow button found")
                                            
                                            if not connection_success:
                                                # Add a note
                                                try:
                                                    wait_span_click(driver, "Add a note", 2)
                                                    message_box = driver.find_element(By.XPATH, "//textarea[@name='message']")
                                                    typing_with_delays(message_box, personalized_message, 0.05, 0.1)
                                                    buffer(1)
                                                    wait_span_click(driver, "Send", 2)
                                                    print_lg(f"Sent connection request to {hr_name}")
                                                    connection_tracker.record_request(hr_name, hr_link, title, company, True, "Request sent")
                                                    connection_success = True
                                                except Exception as note_error:
                                                    # Try sending without note
                                                    try:
                                                        wait_span_click(driver, "Send now", 2)
                                                        print_lg(f"Sent connection request to {hr_name} (without note)")
                                                        connection_tracker.record_request(hr_name, hr_link, title, company, True, "Sent without note")
                                                        connection_success = True
                                                    except:
                                                        raise note_error
                                        ...
                                        print_lg(f"Failed to connect with HR {hr_name}: {error_msg}")
                                        connection_tracker.record_request(hr_name, hr_link, title, company, False, error_msg)
                                
                            except Exception as e:
                                print_lg(f"HR connection error: {str(e)}")
                            finally:
                                if close_tabs: 
                                    try:
                                        driver.close()
                    ...
                    try:
                        ...
                        # Only apply to jobs posted within 1 hour
                        time_text_lower = time_posted_text.lower().strip()
                        
                        # Skip if posted in days, weeks, or months
                        if "day" in time_text_lower or "week" in time_text_lower or "month" in time_text_lower or "year" in time_text_lower:
                            print_lg(f'Job posted {time_posted_text} ago (>1 hour). Skipping.')
                            skip_count += 1
                            continue
                        
                        # Skip if posted more than 1 hour ago
                        if "hour" in time_text_lower:
                            import re
                            numbers = re.findall(r'\d+', time_text_lower)
                            if numbers and int(numbers[0]) > 1:
                                print_lg(f'Job posted {time_posted_text} ago (>1 hour). Skipping.')
                                skip_count += 1
                                continue
                        
                        # Jobs posted in minutes or "1 hour ago" will proceed
                    ...
                    if try_xp(driver, ".//button[contains(@aria-label, 'Easy Apply')]"): easy_apply_clicked = True
                    elif try_xp(driver, ".//button[contains(@class,'jobs-apply-button') and .//span[normalize-space()='Easy Apply']]"): easy_apply_clicked = True
                    elif try_xp(driver, ".//button[contains(@class,'jobs-apply-button') and contains(@class, 'artdeco-button--3') and contains(@aria-label, 'Easy')]"): easy_apply_clicked = True
                    ...
                    if easy_apply_clicked:
                        try: 
                            try:
                                ...
                                while next_button:
                                    next_counter += 1
                                    current_page_source = modal.get_attribute('innerHTML')
                                    if next_counter >= 15: 
                                        if current_page_source == previous_page_source:
                                            ...
                                            # Page content changed, so we are making progress, reset counter or increase limit
                                            print_lg("Page content changed, continuing...")
                                            next_counter = 0 # Reset counter as we moved to a new page/section
                    ...
                    # Track application type and update counters
                    ...
                    # Record analytics
                    if enable_analytics and analytics:
                        try:
                            # Extract applicant count and days posted if available
                            ...
                            analytics.record_application(
                                ...
                                ats_platform=None,  # Will be set for external apps
                                success=True,
                                time_filter=current_time_filter if enable_dynamic_time_filter else "r600",
                                applicant_count=applicant_count,
                                days_posted=days_posted
                            )
                        except Exception as e:
                            print_lg(f"Analytics tracking error: {str(e)}")
                ...
                if elapsed_time > session_duration:
                    print_lg(f"\n\nSession time limit of {session_duration/60:.2f} minutes reached! Stopping search.")
                    return
...
def main() -> None:
    try:
        ...
        msg = "Job application session completed."
...
if __name__ == "__main__":
    # The original main function content is now wrapped in a try block
    # and the restart logic is added.
    # Note: The user's provided 'Code Edit' implies refactoring into
    # initialize_driver(), login_linkedin(), run_search_and_apply().
    # Since the instruction is to make the change faithfully without
    # unrelated edits, the existing main function's body is kept
    # within the new try block.
    try:
        main()
    except Exception as e:
        ...
        with open(log_file, "a", encoding="utf-8") as f:
            f.write(f"[{datetime.datetime.now().isoformat()}] Unexpected error: {e}\n")
            f.write(traceback.format_exc())
            f.write("\n" + "-"*80 + "\n")
        print_lg(f"Error encountered: {e}. Check logs/error_log.txt for details.")
        # Optional: wait before restarting to avoid rapid crash loops
        time.sleep(10)
        # Restart the bot by calling main again (recursive restart)
        main()
