...
def buffer(speed: int=0) -> None:
    ...
    * Will wait within a random range of 
    ...
    if speed<=0:
        ...
        return sleep(randint(6,12)*0.1 + randint(0, 5)*0.01) # 0.6 to 1.25s
    elif speed <= 2 and speed < 3:
        return sleep(randint(10,20)*0.1 + randint(0, 5)*0.01) # 1.0 to 2.05s
    else:
        return sleep(randint(18,round(speed)*10)*0.1 + randint(0, 9)*0.01) # 1.8 to speed+0.09s
